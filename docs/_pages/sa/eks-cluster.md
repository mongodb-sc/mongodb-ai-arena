...
- **VPC**: CIDR block from `aws.vpc_cidr` in `config.yaml`, defaulting to 10.0.0.0/16, with DNS support enabled
- **Subnets**: 2 public /22 subnets carved from the front of the VPC range (10.0.0.0/22, 10.0.4.0/22 by default) across availability zones
...
- **Security**: 
  ...
  - Accessible from VPC (`aws.vpc_cidr`, 10.0.0.0/16 by default)
