...
# Retry settings for Atlas UserWritesBlocked (e.g. DiskUseThresholdExceeded).
# Atlas storage auto-scaling will expand the disk; we wait for writes to resume.
WRITES_BLOCKED_CODE = 371
WRITES_BLOCKED_MAX_WAIT_SECONDS = int(os.environ.get('WRITES_BLOCKED_MAX_WAIT_SECONDS', 30 * 60))
WRITES_BLOCKED_INITIAL_BACKOFF = int(os.environ.get('WRITES_BLOCKED_INITIAL_BACKOFF', 15))
WRITES_BLOCKED_MAX_BACKOFF = int(os.environ.get('WRITES_BLOCKED_MAX_BACKOFF', 120))

def retry_on_writes_blocked(operation, description):
    """Retry a write operation while Atlas reports UserWritesBlocked.
    Other OperationFailure errors are re-raised immediately."""
    start = time.time()
    backoff = WRITES_BLOCKED_INITIAL_BACKOFF
    while True:
        try:
            return operation()
        except OperationFailure as e:
            if e.code != WRITES_BLOCKED_CODE:
                raise
            elapsed = int(time.time() - start)
            if elapsed >= WRITES_BLOCKED_MAX_WAIT_SECONDS:
                print(f"Giving up on '{description}' after {elapsed}s waiting for writes to be unblocked.", file=sys.stderr)
                raise
            print(f"Writes blocked ({e.codeName}) during '{description}'. Sleeping {backoff}s (elapsed {elapsed}s)...", flush=True)
            time.sleep(backoff)
            backoff = min(backoff * 2, WRITES_BLOCKED_MAX_BACKOFF)
...
def create_user_collection(db_name, client, common_database, collections_list):
    for collection in collections_list:
        src = client[common_database][collection]
        retry_on_writes_blocked(
            lambda src=src, coll=collection: src.aggregate([{'$out': {'db': db_name, 'coll': coll}}]),
            f"$out {common_database}.{collection} -> {db_name}.{collection}"
        )
    ...
    for user_id in user_ids:
        try:
            ...
            if user_id in databases:
                # Delete the database
                retry_on_writes_blocked(
                    lambda uid=user_id: client.drop_database(uid),
                    f"drop_database {user_id}"
                )
...
def decommission_unwanted_users(users_map, client, common_database):
    ...
    if to_decommission:
        ...
        result = retry_on_writes_blocked(
            lambda: participants_collection.update_many(
            ...
            "update_many decommission participants"
        )
    ...
    for user_id, user_data in users_map.items():
        ...
        if user_data.get('email') is None:
            ...
            if existing_user:
                ...
                retry_on_writes_blocked(
                    lambda uid=user_id: participants_collection.update_one(
                        {'_id': uid},
                        {'$unset': {'decommissioned': "", 'decommissioned_timestamp': ""}}
                    ),
                    f"update_one participants {user_id} (unset decommissioned)"
                )
            else:
                # New user - insert with decommissioned: false
                participants_document['decommissioned'] = False
                retry_on_writes_blocked(
                    lambda uid=user_id, doc=participants_document: participants_collection.update_one(
                        {'_id': uid},
                        {'$setOnInsert': doc},
                        upsert=True
                    ),
                    f"upsert participants {user_id}"
                )
            # For generated users, still add to user_details collection but without email
            retry_on_writes_blocked(
                lambda uid=user_id, doc=user_details_document: user_details_collection.update_one(
                    {'_id': uid},
                    {'$setOnInsert': doc},
                    upsert=True
                ),
                f"upsert user_details {user_id}"
            )
        else:
            ...
            if existing_user:
                ...
                retry_on_writes_blocked(
                    lambda uid=user_id, name=user_data['name']: participants_collection.update_one(
                        {'_id': uid},
                        {
                            '$set': {'name': name},
                            '$unset': {'decommissioned': "", 'decommissioned_timestamp': ""}
                    ...
                    f"update_one participants {user_id} (set name, unset decommissioned)"
                )
            else:
...
def ensure_results_index(db):
    ...
    if not index_2_exists:
        retry_on_writes_blocked(
            lambda: db["results"].create_index(index_spec_2),
            f"create_index results(name, username) in {db.name}"
        )
    ...
    if not index_3_exists:
        retry_on_writes_blocked(
            lambda: db["results"].create_index(index_spec_3),
            f"create_index results(username, timestamp) in {db.name}"
        )
...
def ensure_participants_indexes(db):
    ...
    for idx in indexes.values():
        if idx.get("key") == index_spec:
            ...
            return
    retry_on_writes_blocked(
        lambda: participants.create_index(index_spec),
        f"create_index participants(taken, taken_timestamp, name) in {participants.database.name}"
    )
...
def create_results_health_collection(db):
    ...
    if collection_name not in existing_collections:
        ...
        retry_on_writes_blocked(
            ...
            f"create_collection {collection_name} in {db.name}"
        )
...
def create_regular_index(collection, index_name, index_definition):
    ...
    try:
        ...
        # Create the index
        retry_on_writes_blocked(
            lambda: collection.create_index(list(index_definition.items()), name=index_name),
            f"create_index '{index_name}' in {collection.database.name}.{collection.name}"
        )
