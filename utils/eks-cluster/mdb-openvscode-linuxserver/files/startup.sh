...
if [ "$(is_enabled '.database.postgres')" = "true" ] && [ "$(is_enabled '.llm.mcp')" = "true" ]; then
    ...
    sudo -u abc bash -c 'uv venv'
    sudo -u abc bash -c 'uv pip install postgres-mcp'
else
...
if [ ! -d "/home/workspace/$REPO_NAME" ]; then
    ...
    chown -R abc:abc /home/workspace/$REPO_NAME
else
...
if ! sudo -u abc bash /tmp/user_operations.sh; then
