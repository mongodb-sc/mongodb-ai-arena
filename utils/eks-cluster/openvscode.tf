...
resource "helm_release" "user_openvscode" {
  ...
  repository = "local"
  chart      = "./mdb-openvscode-linuxserver"
  version    = "0.2.23"
  timeout    = 600

  values = [
    file("${path.module}/mdb-openvscode-linuxserver/values.yaml"),
    yamlencode(merge({
      podAnnotations = {
        "scenario-checksum" = sha256(jsonencode({
          ...
          vscode       = try(var.scenario_config.vscode, {})
          cline        = try(var.scenario_config.cline, {})
        }))
      },
      env = [
        {
        ...
        {
          # Point Cline at the EFS-backed workspace instead of $HOME/.cline, so the
          # config written by the init container is visible to the main container
          # (/config is a per-container image layer, not a shared volume).
          name  = "CLINE_DIR"
          value = "/home/workspace/.cline"
        },
        {
          name  = "MONGODB_URI"
      ...
      volumeMounts = [
        ...
        },
        {
          ...
          mountPath = "/config/.openvscode-server/data/User/globalStorage/saoudrizwan.claude-dev/settings"
        },
        {
