  ...
  values = [
    yamlencode({
      ...
      env = [
        {
          name  = "MONGODB_URI"
          value = "mongodb+srv://${local.atlas_admin_user}:${local.atlas_admin_password}@${replace(local.atlas_private_srv, "mongodb+srv://", "")}/?retryWrites=true&w=majority"
        },
        {
          name  = "DB_NAME"
          value = "arena_shared"
