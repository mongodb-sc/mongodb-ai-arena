...
def load_nav_labels(lang):
    """Load nav_labels translation dictionary for a language from the docs data directory."""
    label_paths = [
        os.getenv("NAV_LABELS_PATH", ""),  # env override
    ]
    nav_path = os.getenv("NAVIGATION_FILE_PATH", "")
    if nav_path:
        label_paths.append(os.path.join(os.path.dirname(nav_path), "nav_labels", f"{lang}.yml"))
    label_paths.extend([
        os.path.join(os.path.dirname(__file__), "../../../../docs/_data/nav_labels", f"{lang}.yml"),
        f"docs/_data/nav_labels/{lang}.yml",
    ])
    for path in label_paths:
        if path and os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                return yaml.safe_load(f) or {}
    print(f"Warning: nav_labels file for '{lang}' not found. Tried: {label_paths}")
    return {}


def derive_multilang_navigation(nav_content):
    """Derive docs_es / docs_pt (and guided variants) from docs using nav_labels dictionaries."""
    if not nav_content:
        return nav_content
    ...
    for lang, prefix in languages.items():
        labels = load_nav_labels(lang)

        def translate(title):
            return labels.get(title, title)

        def prefix_url(url):
            # Only prefix relative URLs (not external http/https links)
            if url and url.startswith("/") and not url.startswith("//"):
                return prefix.rstrip("/") + url
            return url

        def translate_section(section):
            new_section = {"title": translate(section.get("title", ""))}
            if "url" in section:
                new_section["url"] = prefix_url(section["url"])
            if "children" in section:
                new_section["children"] = [translate_section(c) for c in section["children"]]
            return new_section

        # Derive docs_<lang> from docs
        if "docs" in nav_content:
            nav_content[f"docs_{lang}"] = [translate_section(s) for s in nav_content["docs"]]
            print(f"Derived docs_{lang} with {len(nav_content[f'docs_{lang}'])} sections")

        # Derive docs_guided_<lang> from docs_guided (if present)
        if "docs_guided" in nav_content:
            nav_content[f"docs_guided_{lang}"] = [translate_section(s) for s in nav_content["docs_guided"]]
            print(f"Derived docs_guided_{lang} with {len(nav_content[f'docs_guided_{lang}'])} sections")
...
def _apiserver_base_url():
    """Return the in-cluster apiserver URL from the standard env vars."""
...
def _sa_auth_headers():
    """Read the SA token fresh from disk and return Authorization headers.

    Reading on every call (instead of caching) sidesteps stale-token issues in
    long-running init containers and known bugs in some versions of the
    kubernetes python client where the bearer token is not attached to the
    request at all.
    """
...
def create_enhanced_configmap(enhanced_config):
    """Create or update the enhanced ConfigMap by calling the apiserver directly via requests."""
    try:
        ...
        body = {
            ...
            "data": {
                "enhanced-scenario-config.json": json.dumps(enhanced_config, indent=2, default=str)
            },
        }

        base = _apiserver_base_url()
        url = f"{base}/api/v1/namespaces/{namespace}/configmaps/{configmap_name}"
        collection_url = f"{base}/api/v1/namespaces/{namespace}/configmaps"

        def _put_or_create():
            headers = _sa_auth_headers()
            headers["Content-Type"] = "application/json"
            # Try PUT (update) first; on 404 fall back to POST (create).
            r = requests.put(url, headers=headers, data=json.dumps(body), verify=SA_CA_PATH, timeout=30)
            if r.status_code == 404:
                headers = _sa_auth_headers()
                headers["Content-Type"] = "application/json"
                r = requests.post(collection_url, headers=headers, data=json.dumps(body), verify=SA_CA_PATH, timeout=30)
                if r.status_code not in (200, 201):
                    raise RuntimeError(f"POST {collection_url} -> HTTP {r.status_code}: {r.text[:300]}")
        ...
        try:
            ...
            if "HTTP 401" in str(e):
                print("Got 401 from Kubernetes API, re-reading SA token from disk and retrying once...")
                _put_or_create()
            else:
...
def main():
    ...
    # Derive multilingual navigation keys (docs_es, docs_pt, docs_guided_es, docs_guided_pt)
    # from the base docs key using nav_labels translation dictionaries.
    if navigation_content:
        navigation_content = derive_multilang_navigation(navigation_content)
        print("Multilingual navigation keys derived.")
    ...
    # This ConfigMap is mounted by every vscode pod, so failing here must abort
    # the init container so the deployment retries instead of silently leaving
    # downstream pods stuck on a missing volume.
    ...
    if configmap_success:
        ...
        print("Failed to create/update enhanced ConfigMap, exiting...")
        sys.exit(1)
